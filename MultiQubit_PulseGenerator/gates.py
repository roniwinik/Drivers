...
class CRGate_pulse(SingleQubitXYRotation):
    """Creates the CR pulse gate used in the CR gate.
...
class CrossResonance(CompositeGate):
    """Cross-resonance gate followed by single qubit Z rotations.
    Parameters
    ----------
    amp1 : float
        Drive amplitude, qubit 1.
    plateau1 : float
        Drive duration, qubit 1.
    phase1 : float
        Drive phase, qubit 1.
    ampCR : float
        Drive amplitude, AWG CR.
    plateauCR : float
        Drive duration, AWG CR.
    phaseCR : float
        Drive phase, AWG CR.
    phi_z1 : float
    ...
    """
    def __init__(self, cr_amp, cr_length, cr_phase, crc_amp, crc_phase,
                 phi_z1, phi_z2):
        super().__init__(n_qubit=3)
        self.add_gate(
            [RabiGate(crc_amp, cr_length, crc_phase),
            I,
            RabiGate(cr_amp, cr_length, cr_phase)]
        )
        #self.add_gate([VirtualZGate(phi_z1), VirtualZGate(phi_z1)], I)
    def update_params(self, cr_amp, cr_length, cr_phase, crc_amp, crc_phase,
                 phi_z1, phi_z2):
        ...
        phi_z2: float
            Z rotation angle for qubit 2.
        """
        self.__init__(cr_amp, cr_length, cr_phase, crc_amp, crc_phase,
                 phi_z1, phi_z2)
    def __str__(self):
        return "CR"


class CRgate(CompositeGate):
    """CR gate.
